...
Lưu Ý! Đây Là Sản Phẩm Được VĐT Remake ( Chính Bởi Facebook-Chat-Api Của Schmavery, Tác Giả Không Chịu Trách Nghiệm Nào ! ), Nếu Có Lỗi Hãy Thử Sử Dụng Sang Sản Phẩm Khác !
...
```bash
npm i fca-zera-remake
```
or
```bash
npm install fca-zera-remake
```
...
```bash
npm install fca-zera-remake@latest
```
Hoặc
```bash
npm i fca-zera-remake@latest
```
...
```javascript
const login = require("fca-zera-remake"); // lấy từ lib ra 
...
```js
    ...
    /* Có thể là :
        ...
        var login = require('@xaviabot/fca-unofficial');
...
```js
    var login = require('fca-zera-remake')
```
