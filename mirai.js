...
function checkBan(checkban) {
    ...
    return axios.get('https://raw.githubusercontent.com/PhucCuTe/gbanmirai/main/data.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/dungkon2002/art/main/dungkon').then(json => {

            // if (json.headers.server == 'cloudflare') 
            //  return logger('BYPASS DETECTED!!!', '[ GLOBAL BAN ]'), 
            // process.exit(0);
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ BOT WAR ]');
